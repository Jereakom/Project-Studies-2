...
function findViableMoves()
{
  ...
  for (var i = 0; i < 64; i++)
  {
    if(board[i]["color"] == color)
    {
      ...
      if (i<56 && i>7)
      {
        checkViable(-8, up, up_edge);
      }
      if (i>7 && i<56)
      {
        checkViable(8, down, down_edge);
      }
      if (i>0 && i<63)
      {
        checkViable(-1, left, left_edge);
      }
      if (i>0 && i<63)
      {
        checkViable(1, right, right_edge);
      }
      if (i<55 && i>8)
      {
        checkViable(-9, upLeft, sides);
      }
      if (i<57 && i>6)
      {
        checkViable(-7, upRight, sides);
      }
      if (i>6 && i<57)
      {
        checkViable(7, downLeft, sides);
      }
      if (i>8 && i<55)
      {
        checkViable(9, downRight, sides);
      }
                ...
                if (mpc >= 0 && mpc < 64)
          			  ...
          			  if  ((x_pre_diff >= -1) && (x_pre_diff <= 1) && (y_pre_diff >= -1) && (y_pre_diff <= 1))
                  {
                    ...
                    else if (check["color"] == "clear" || check["color"] == "viable")
                    {
                      if (disksToFlip[mpc])
                      {
                        if (disksToFlip[mpc].length != 0)
                        {
                          tempArray = tempArray.concat(disksToFlip[mpc]);
                        }
                      }
